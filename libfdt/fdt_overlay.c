 ...
 *      Negative error code on failure
 */
static int overlay_fixup_one_path_or_phandle(
	const void *fdt, void *fdto, int symbols_off, const char *path,
	uint32_t path_len, const char *name, uint32_t namelen, int poffset,
	const char *symbol_path, int *goffset)
{
	const void *prop;
	void *prop_rw;
	...
	if (prop_len % sizeof(phandle_empty) == 0 &&
	    memcmp((const char *)prop + poffset, &phandle_empty,
		   sizeof(phandle_empty)) == 0) {
		...
		ret = fdt_setprop_placeholder_namelen(
			fdto, fixup_off, name, namelen, new_prop_len, &prop_rw);
		if (ret)
			return ret;

		memmove((char *)prop_rw + poffset + symbol_path_len,
			(char *)prop_rw + poffset, prop_len - poffset);
		memcpy((char *)prop_rw + poffset, symbol_path, symbol_path_len);

		*goffset = symbol_path_len + 1;
					 ...
					 int *grow_offset)
{
	...
	do {
		...
		/* Not really sure why but things fail without this check */
		if (goffset && value[goffset] == '\0')
			goffset++;

		/* Assume that __fixups__ is always after all the fragments */
		value += goffset;
		*grow_offset += goffset;
	} while (len > 0);
...
{
	int fixups_off, symbols_off, property, ret, goffset;
	...
	fdt_for_each_property_offset(property, fdto, fixups_off) {
		...
		ret = overlay_fixup_path_or_phandle(fdt, fdto, symbols_off,
						    property, &goffset);
		if (ret)
			return ret;
